// industrial-iot-lab/dashboard-industrial/src/components/motor/OilSystemDualChart.tsx
"use client";
...
type OilRow = {
  ts: number;
  tempAceite: number | null;
  tempFiltro: number | null;
  presion: number | null;
};
...
function OilSystemPlot({ data, height = 300 }: OilSystemPlotProps) {
  ...
  // ✅ Calcular rango X basado en followSec
  ...
  const computeYRangeForVisibleX = useCallback(
    (startMs: number, endMs: number): [number, number] | null => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        const temps = [row.tempAceite, row.tempFiltro].filter(
  ...
  const [defaultTempMin, defaultTempMax] = useMemo<[number, number]>(() => {
    if (tempsAll.length === 0) return [0, 120];
  ...
  const [defaultPresMin, defaultPresMax] = useMemo<[number, number]>(() => {
    if (presAll.length === 0) return [0, 3];
  ...
  const traces: Data[] = useMemo(
    () => [
      {
        x: xValues,
        y: toSeries(data.map((d) => d.tempAceite)),
        type: "scatter",
        mode: "lines",
        name: "Temp. Aceite",
        line: {
        ...
        hovertemplate: "Temp. Aceite: %{y:.2f} °C<extra></extra>",
        yaxis: "y",
      },
      {
        x: xValues,
        y: toSeries(data.map((d) => d.tempFiltro)),
        type: "scatter",
        mode: "lines",
        name: "Temp. Filtro",
        line: {
        ...
        hovertemplate: "Temp. Filtro: %{y:.2f} °C<extra></extra>",
        yaxis: "y",
      },
      {
        x: xValues,
        ...
        hovertemplate: "Presión: %{y:.2f} bar<extra></extra>",
        yaxis: "y2",
      },
    ],
    [data, xValues]
  ...
  () => ({
    ...
    uirevision: `oil_v1_fs${followSec ?? "auto"}_${
...
export function OilSystemDualChart({
  ...
  const chartRows: OilRow[] = useMemo(() => {
  ...
  // Estados individuales para las 3 cajas
  const aceiteStatus = getTempStatus(tempAceite, 90, 5);
  const filtroStatus = getTempStatus(tempFiltro, 50, 5);
  const presionStatus = getPressureStatus(presionAceite, 1.0, 0.8);
    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mb-4">
        ...
        >
          ...
          {renderStatusBadge(aceiteStatus)}
        </div>
        ...
        >
          ...
          {renderStatusBadge(filtroStatus)}
        </div>
      ...
      <OilSystemPlot data={chartRows} height={300} />
        ...
        <div className="text-xs text-gray-600 space-y-1">
          <div className="flex justify-between">
            ...
            <span className="font-mono">
              Aceite: 80-90°C | Filtro &lt; 50°C | Presión: 1.0-2.0 bar
            </span>
          </div>
        </div>
      </div>
