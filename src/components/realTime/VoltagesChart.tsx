// industrial-iot-lab/dashboard-industrial/src/components/VoltagesChart.tsx
"use client";
...
export const DEFAULT_VOLT_MIN = 260;
export const DEFAULT_VOLT_MAX = 290;
...
// Toolbar (Banda) con mismo look que Currents
type ToolbarProps = {
  onToggleBand: () => void;
  bandActive: boolean;
...
export default function VoltagesChart({
  data,
  height = 380,
  yRange,
  thresholds,
}: Props) {
  // --- Refs principales (normal) ---
  ...
  // --- Umbrales ---
  const defaultMin = thresholds?.min ?? DEFAULT_VOLT_MIN;
  const defaultMax = thresholds?.max ?? DEFAULT_VOLT_MAX;

  // Banda APAGADA por defecto (igual que Currents)
  ...
  const computeYRangeForVisibleX = useCallback(
    (startMs: number, endMs: number): [number, number] | null => {
      ...
      if (minVal === maxVal) {
        const delta = Math.max(0.5, Math.abs(minVal) * 0.01);
  ...
  // Rango Y datos visibles para 30s/1m/5m
  ...
  const resetToDefaultView = useCallback(
    (gd: PlotlyHTMLElement | null) => {
      ...
      // 5) Relayout directo (sin autorange)
      if (gd) {
  ...
  // --- Series (mantengo nombres/colores de Voltajes) ---
  const traces: Data[] = useMemo((): Data[] => {
    ...
    return [
      {
        x: safeX,
        y: yOrNaN(data.map((d) => d["L1-N"] ?? NaN)),
        type: "scatter",
        mode: "lines",
        name: "L1-N",
        showlegend: true,
        line: { color: "#06b6d4", width: 1.8, shape: "spline", simplify: true },
        hovertemplate: "L1-N: %{y:.2f} V<extra></extra>",
      },
      {
        x: safeX,
        y: yOrNaN(data.map((d) => d["L2-N"] ?? NaN)),
        type: "scatter",
        mode: "lines",
        name: "L2-N",
        showlegend: true,
        line: { color: "#f59e0b", width: 1.8, shape: "spline", simplify: true },
        hovertemplate: "L2-N: %{y:.2f} V<extra></extra>",
      },
      {
        x: safeX,
        y: yOrNaN(data.map((d) => d["L3-N"] ?? NaN)),
        type: "scatter",
        mode: "lines",
        name: "L3-N",
        showlegend: true,
        line: { color: "#8b5cf6", width: 1.8, shape: "spline", simplify: true },
        hovertemplate: "L3-N: %{y:.2f} V<extra></extra>",
      },
      {
        x: safeX,
        ...
        hovertemplate: "Promedio: %{y:.2f} V<extra></extra>",
      },
    ];
  }, [xDates, data]);
  ...
  const thrShapes: Partial<Shape>[] = useMemo(() => {
    ...
    // Líneas de umbral (rojas punteadas)
    shapes.push(
      {
        type: "line",
        xref: "paper",
  ...
  useEffect(() => {
    ...
    const placeUIStable = () => {
      ...
      // Margen derecho fijo (como Currents)
      const desiredRight = 96;
      if (rightMargin !== desiredRight) setRightMargin(desiredRight);

      // Columna del modebar ANCLADA arriba-derecha (dejamos espacio para el botón Ampliar)
      if (modebar) {
      ...
      // Botón Banda: arriba-izquierda (look del rangeselector)
      if (bandaHost) {
  ...
  useEffect(() => {
    ...
    const placeUIStable = () => {
      ...
      // Columna del modebar anclada arriba-derecha (con espacio para Ampliar)
      if (modebar) {
        modebar.style.display = "flex";
        modebar.style.flexDirection = "column";
        modebar.style.alignItems = "center";
  ...
  const onAfterPlotCommon = (root: HTMLElement | null) => {
    hookRangeSelectorClicks(
      root,
      root === containerRef.current ? boundButtons.current : fsBoundButtons.current
    );
  ...
  const modal =
    ...
    createPortal(
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Gráfica de Voltajes - Vista ampliada"
      >
  ...
  }, [isFullscreen, followSec, hookRangeSelectorClicks, paintButtons, labelFromFollow]);

  useEffect(() => {
    paintButtons(containerRef.current, labelFromFollow(followSec));
    if (isFullscreen) paintButtons(fsContainerRef.current, labelFromFollow(followSec));
  ...
  return (
    <>
      <div className="relative w-full" ref={containerRef}>
        ...
        {/* Botón Ampliar como el primero del modebar */}
        <div
